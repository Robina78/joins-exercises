...
--The first_name should be ordered in descending order. Only display results with more
-- than one vehicle and an average price greater than 10000. 
SELECT first_name, last_name, round(AVG(price)) AS average_price, COUNT(owner_id)
    FROM owners JOIN vehicles ON owners.id = vehicles.owner_id
    GROUP BY (last_name, first_name) HAVING COUNT(owner_id) > 1 AND ROUND(AVG(price)) > 10000
